...
- (void)viewDidLoad {
...
#if 0	// Changed Linphone code - Display different version info
	...
	_appVersionLabel.text = [NSString stringWithFormat:@"Version:   iOS  %@", version];
	_libVersionLabel.text = [NSString stringWithFormat:@"Build:   %@", build];
#endif
...
#if 0	// Changed Linphone code - Display different URL for privacy policy
	NSString *url = @"http://www.linphone.org/privacy-policy.html";
#else
	NSString *url = @"https://snrblabs.com/GrooVeIp/PrivacyPolicy";
#endif
	if (![UIApplication.sharedApplication openURL:[NSURL URLWithString:url]]) {
		LOGE(@"Failed to open %@, invalid URL", url);
	}
