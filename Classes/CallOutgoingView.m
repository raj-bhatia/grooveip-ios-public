...
- (void)viewWillAppear:(BOOL)animated {
	...
	ms_free(uri);
#else
	BOOL foundContact = [ContactDisplay setDisplayNameLabel:_nameLabel forAddress:addr];
	if (YES == foundContact) {
		char *uri = linphone_address_as_string_uri_only(addr);
		NSString *phoneNumberOnly = [LinphoneUtils phoneNumberFromSipUri: [NSString stringWithUTF8String:uri]];
		_addressLabel.text = phoneNumberOnly;
		ms_free(uri);
	} else {
		_addressLabel.text = @"";
	}
...
#if 0	// Changed Linphone code - Need to call these from UI thread otherwise get exceptions
