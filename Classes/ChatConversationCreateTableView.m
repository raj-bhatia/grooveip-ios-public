...
#if 0	// Changed Linphone code - Don't convert filter string to SIP URI
...
#else
	uri = [_contacts.allKeys objectAtIndex:indexPath.row];
	NSString *normalizedNumber = [LinphoneUtils normalizePhoneNumber: uri];
	
	LinphoneChatRoom *room = linphone_core_get_chat_room_from_uri(LC, normalizedNumber.UTF8String);
#endif
	if (!room) {
