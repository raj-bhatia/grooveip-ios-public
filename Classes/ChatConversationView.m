...
#if 1	// Changed Linphone code - Method for displaying error message
...
#if 1	// Changed Linphone code - Send SMS directly to SMS server using API
	const LinphoneAddress *addr = linphone_chat_room_get_peer_address(_chatRoom);
	if (addr != NULL) {
		...
		if (to && uid && tkn && frm) {
			...
			NSLog(@"Sending SMS: UserId %@ Token %@ To %@ From %@ Text %@", userId, token, to, from, message);
		
			SmsManager * smsManager = [SmsManager instance];
			[smsManager smsWithUserId:userId token:token to:to from:from text:message
				...
				if (0 != *status) {
					...
					NSLog(@"SMS sending error: %d", *status);
					return;
				}
			
				// Received 200 OK
				dispatch_async(dispatch_get_main_queue(), ^{
					linphone_chat_message_update_state(msg, LinphoneChatMessageStateDelivered);
				});
			}];
		} else {
			linphone_chat_message_update_state(msg, LinphoneChatMessageStateNotDelivered);
			NSLog(@"Error - SMS not sent: UserId %s Token %s To %@ From %s Text %@", uid, tkn, to, frm, message);
			return FALSE;
		}
	}
#endif
...
- (void)chooseImageQuality:(UIImage *)image url:(NSURL *)url {
	...
	dispatch_async(dispatch_get_main_queue(), ^{
		...
		[sheet addButtonWithTitle:text
							block:^() {
								[self saveAndSend:image url:url withQuality:0];	// Value of quality is don't care
							}];
		[sheet addCancelButtonWithTitle:NSLocalizedString(@"Cancel", nil) block:nil];
		[sheet showInView:PhoneMainView.instance.view];
	});
...
#if 0	// Changed Linphone code - Difference between chat and SMS--no need to process "user is composing" indication
