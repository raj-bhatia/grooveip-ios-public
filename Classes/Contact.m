...
#if 1	// Changed Linphone code - For debugging only
static NSInteger totalContacts = 0;
static NSInteger validContacts = 0;
#endif
...
- (instancetype)initWithPerson:(CNContact *)acncontact
  ...
  self = [super init];
  _person = acncontact;
#if 0	// Changed Linphone code - No friend, sipAddress or email for GrooVe IP
  ...
  */
#else
  LOGI(@"Contact %@ %@ initialized with %d phones. Total = %d, Valid = %d",
       self.firstName ?: @"", self.lastName ?: @"", self.phones.count, totalContacts, validContacts);
#endif
  return self;
}
...
#else
	_firstName = firstName;
	_displayName = [NSString stringWithFormat:@"%@ %@", firstName, _lastName];
#endif
}
