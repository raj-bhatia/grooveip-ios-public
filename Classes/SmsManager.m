...
static SmsManager *smsManager = nil;
...
{
	LOGD(@"SmsManager enter: userId %@ token %@ to %@ from %@ text %@", userId, token, to, from, text);
	...
	[request setURL:[NSURL URLWithString:SMS_URL]];
	...
	NSString *hash = [Secret getHash_Sms:userId token:token from:from to:to text:text epochTime:nowEpochMSecs];
	...
	[[session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
		...
		[SmsManager parseResponse : data : smsResponse : error];
...
+ (SmsManager*)instance {
	@synchronized(self) {
		if (smsManager == nil) {
			smsManager = [[SmsManager alloc] init];
		}
	}
	return smsManager;
}
