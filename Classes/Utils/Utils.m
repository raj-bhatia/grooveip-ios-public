...
+ (NSString *) normalizePhoneNumber:(NSString *)inputString {
	...
	const char *in = [inputString UTF8String];
	...
	if ((size == 10) && (firstChar != '+')) {
		strcpy(outputBuffer, "+1");
		strcat(outputBuffer, temp);
	} else if ((size == 11) && (firstChar == '1')) {
		strcpy(outputBuffer, "+");
		strcat(outputBuffer, temp);
	} else {
...
+ (NSString *) phoneNumberFromSipUri: (NSString *) uri {
	NSUInteger start = [uri rangeOfString: @"sip:"].length;
	if (start != 4) {
		return uri;
	}
	NSUInteger end = [uri rangeOfString: @"@"].location;
	if (end > 32) {
		return uri;
	}
...
#if 0	// Changed Linphone code - Return a BOOL to indicate if a matching contact was found
