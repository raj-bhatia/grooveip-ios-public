...
void snrblabs_report_call_log(LinphoneCore *lc, char *number, char *name, int direction, int duration, long startTime, long connectTime, int status, char *callId) {
	ms_message("%s(): number %s name %s direction %i duration %i start %ld connect %ld status %i callId %s",
		__FUNCTION__, number, name, direction, duration, startTime, connectTime, status, callId);

	char buffer [64];
	sprintf (buffer, "sip:%s@fr.com", number);
	LinphoneAddress *f = linphone_address_new(buffer);
	linphone_address_set_display_name(f, name);
	sprintf (buffer, "sip:%s@to.com", number);
	LinphoneAddress *t = linphone_address_new(buffer);
	linphone_address_set_display_name(t, name);
	LinphoneCallLog *log = linphone_call_log_new((LinphoneCallDir) direction, f, t);
	log->duration = duration;
	log->start_date_time = startTime;
	log->connected_date_time = connectTime;
	log->status = (LinphoneCallStatus) status;
	log->video_enabled = 0;
	log->quality = 5.0;
	log->call_id = callId;
	log->refkey = NULL;

	linphone_core_report_call_log(lc, log);
