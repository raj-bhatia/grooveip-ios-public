...
static void linphone_proxy_config_init(LinphoneCore* lc, LinphoneProxyConfig *cfg) {
	...
	const char *snrblabs_email = lc ? lp_config_get_default_string(lc->config, "proxy", "snrblabs_email", NULL) : NULL;
	const char *snrblabs_userid = lc ? lp_config_get_default_string(lc->config, "proxy", "snrblabs_userid", NULL) : NULL;
	const char *snrblabs_token = lc ? lp_config_get_default_string(lc->config, "proxy", "snrblabs_token", NULL) : NULL;
	const char *snrblabs_devicetoken = lc ? lp_config_get_default_string(lc->config, "proxy", "snrblabs_devicetoken", NULL) : NULL;
	cfg->lc = lc;
	...
	cfg->snrblabs_email = snrblabs_email ? ms_strdup(snrblabs_email) : NULL;
	cfg->snrblabs_userid = snrblabs_userid ? ms_strdup(snrblabs_userid) : NULL;
	cfg->snrblabs_token = snrblabs_token ? ms_strdup(snrblabs_token) : NULL;
	cfg->snrblabs_devicetoken = snrblabs_devicetoken ? ms_strdup(snrblabs_devicetoken) : NULL;
	cfg->snrblabs_currency = lc ? lp_config_get_default_int(lc->config, "proxy", "snrblabs_currency", 0) : 0;
	if (nat_policy_ref) {
...
void _linphone_proxy_config_destroy(LinphoneProxyConfig *cfg){
	...
	if (cfg->snrblabs_email) ms_free(cfg->snrblabs_email);
	if (cfg->snrblabs_userid) ms_free(cfg->snrblabs_userid);
	if (cfg->snrblabs_token) ms_free(cfg->snrblabs_token);
	if (cfg->snrblabs_devicetoken) ms_free(cfg->snrblabs_devicetoken);
...
void linphone_proxy_config_write_to_config_file(LpConfig *config, LinphoneProxyConfig *cfg, int index)
{
	...
	if (cfg->snrblabs_email) lp_config_set_string(config,key,"snrblabs_email",cfg->snrblabs_email);
	if (cfg->snrblabs_userid) lp_config_set_string(config,key,"snrblabs_userid",cfg->snrblabs_userid);
	if (cfg->snrblabs_token) lp_config_set_string(config,key,"snrblabs_token",cfg->snrblabs_token);
	if (cfg->snrblabs_devicetoken) lp_config_set_string(config,key,"snrblabs_devicetoken",cfg->snrblabs_devicetoken);
	lp_config_set_int(config,key,"snrblabs_currency",cfg->snrblabs_currency);
...
LinphoneProxyConfig *linphone_proxy_config_new_from_config_file(LinphoneCore* lc, int index)
{
	...
	CONFIGURE_STRING_VALUE(cfg,config,key,snrblabs_email,"snrblabs_email")
	CONFIGURE_STRING_VALUE(cfg,config,key,snrblabs_userid,"snrblabs_userid")
	CONFIGURE_STRING_VALUE(cfg,config,key,snrblabs_token,"snrblabs_token")
	CONFIGURE_STRING_VALUE(cfg,config,key,snrblabs_devicetoken,"snrblabs_devicetoken")
	CONFIGURE_INT_VALUE(cfg,config,key,snrblabs_currency,"snrblabs_currency", int)
...
void linphone_proxy_config_set_snrblabs_email (LinphoneProxyConfig *cfg, const char *value) {
	if (cfg->snrblabs_email != NULL){
		ms_free (cfg->snrblabs_email);
		cfg->snrblabs_email = NULL;
	}
	if (value) cfg->snrblabs_email = ms_strdup(value);
}

const char *linphone_proxy_config_get_snrblabs_email (const LinphoneProxyConfig *cfg) {
	return cfg->snrblabs_email;
}

void linphone_proxy_config_set_snrblabs_userid (LinphoneProxyConfig *cfg, const char *value) {
	if (cfg->snrblabs_userid != NULL){
		ms_free (cfg->snrblabs_userid);
		cfg->snrblabs_userid = NULL;
	}
	if (value) cfg->snrblabs_userid = ms_strdup(value);
}

const char *linphone_proxy_config_get_snrblabs_userid (const LinphoneProxyConfig *cfg) {
	return cfg->snrblabs_userid;
}

void linphone_proxy_config_set_snrblabs_token (LinphoneProxyConfig *cfg, const char *value) {
	if (cfg->snrblabs_token != NULL){
		ms_free (cfg->snrblabs_token);
		cfg->snrblabs_token = NULL;
	}
	if (value) cfg->snrblabs_token = ms_strdup(value);
}

const char *linphone_proxy_config_get_snrblabs_token (const LinphoneProxyConfig *cfg) {
	return cfg->snrblabs_token;
}

void linphone_proxy_config_set_snrblabs_devicetoken (LinphoneProxyConfig *cfg, const char *value) {
	if (cfg->snrblabs_devicetoken != NULL){
		ms_free (cfg->snrblabs_devicetoken);
		cfg->snrblabs_devicetoken = NULL;
	}
	if (value) cfg->snrblabs_devicetoken = ms_strdup(value);
}

const char *linphone_proxy_config_get_snrblabs_devicetoken (const LinphoneProxyConfig *cfg) {
	return cfg->snrblabs_devicetoken;
}

void linphone_proxy_config_set_snrblabs_currency (LinphoneProxyConfig *cfg, int value) {
	cfg->snrblabs_currency = value;
}

int linphone_proxy_config_get_snrblabs_currency (const LinphoneProxyConfig *cfg) {
	return cfg->snrblabs_currency;
}
