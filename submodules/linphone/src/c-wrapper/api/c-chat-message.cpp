...
void linphone_chat_message_update_state (LinphoneChatMessage *msg, const LinphoneChatMessageState state) {
	if (LinphoneChatMessageStateNotDelivered == state) {
		L_GET_PRIVATE_FROM_C_OBJECT(msg)->setState(LinphonePrivate::ChatMessage::State::NotDelivered);
	} else if (LinphoneChatMessageStateDelivered == state) {
		L_GET_PRIVATE_FROM_C_OBJECT(msg)->setState(LinphonePrivate::ChatMessage::State::Delivered);
	} else if (LinphoneChatMessageStateInProgress == state) {
		L_GET_PRIVATE_FROM_C_OBJECT(msg)->setState(LinphonePrivate::ChatMessage::State::InProgress);
	} else if (LinphoneChatMessageStateFileTransferDone == state) {
		L_GET_PRIVATE_FROM_C_OBJECT(msg)->setState(LinphonePrivate::ChatMessage::State::FileTransferDone);
	} else if (LinphoneChatMessageStateDisplayed == state) {
		L_GET_PRIVATE_FROM_C_OBJECT(msg)->setState(LinphonePrivate::ChatMessage::State::Displayed);
	}
}
#endif
