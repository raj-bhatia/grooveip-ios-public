...
LinphoneChatMessage *linphone_chat_room_create_message_incoming (LinphoneChatRoom *cr, const char *message) {
	shared_ptr<LinphonePrivate::ChatMessage> cppPtr = L_GET_CPP_PTR_FROM_C_OBJECT(cr)->createChatMessageIncoming(L_C_TO_STRING(message));
	LinphoneChatMessage *object = L_INIT(ChatMessage);
	L_SET_CPP_PTR_FROM_C_OBJECT(object, cppPtr);
	return object;
}

LinphoneChatMessage *linphone_chat_room_create_message_incoming_url (LinphoneChatRoom *cr, const char *message, const char *url) {
	LinphoneChatMessage *msg = linphone_chat_room_create_message_incoming(cr, message);
	linphone_chat_message_set_external_body_url(msg, url ? ms_strdup(url) : NULL);
...
#if 0	// Changed Linphone code - SMS/MMS
	L_GET_PRIVATE_FROM_C_OBJECT(msg)->receive();
#else
	L_GET_PRIVATE_FROM_C_OBJECT(msg)->notifyReceiving();
#endif
}
