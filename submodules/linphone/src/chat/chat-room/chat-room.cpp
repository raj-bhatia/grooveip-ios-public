...
shared_ptr<ChatMessage> ChatRoom::createChatMessageIncoming () {
	L_D();
	return d->createChatMessage(ChatMessage::Direction::Incoming);
}

shared_ptr<ChatMessage> ChatRoom::createChatMessageIncoming (const string &text) {
	shared_ptr<ChatMessage> chatMessage = createChatMessageIncoming();
	Content *content = new Content();
	content->setContentType(ContentType::PlainText);
	content->setBody(text);
	chatMessage->addContent(content);
